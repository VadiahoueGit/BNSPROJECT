...
@Component({
  selector: 'app-ecart-emballage',
  templateUrl: './ecart-emballage.component.html',
  styleUrls: ['./ecart-emballage.component.scss']
})
export class EcartEmballageComponent {
...
// Recalcul de la cargaison après modification de regroupementFinal
  ...
  GetEcartEmbalageList(page: number, nom?: string): Promise<void> {
    let data = {
      paginate: true,
      page: page,
      limit: 8,
    ...
    ALERT_QUESTION(
      ...
      if (res.isConfirmed == true) {
        ...
        this._activite.TerminerRegroupement(data.regroupementId).then((res: any) => {
          ...
          this._spinner.hide();
          this.GetEcartEmbalageList(1).then(() => {
  ...
  onPage(event: any) {
    ...
    this.rowsPerPage = event.rows;
    this.GetEcartEmbalageList(this.currentPage);
  }
